...
export async function completePasswordReset({ userId, newPassword }) {
  if (!userId || !newPassword || newPassword.length < 6) {
    throw new Error('User ID and a new password (min 6 characters) are required')
  }
  const { data: user, error: fetchErr } = await sb.from('users').select('*').eq('id', userId).maybeSingle()
